...
"""
    ...
    if sorted_fails:
      ...
      <div class="fail-main">
        <span class="fail-addr">{_esc(addr)}</span>
...
"""
    ...
    <strong>발송 실패 추적</strong> · 이메일 주소별 누적 발송 실패 횟수입니다. 반복 실패하는 주소는 주소록에서 정리를 검토하세요. 소프트바운스는 재시도로 이후 성공할 수 있어 조회 시점 기준입니다.{(' · 갱신: ' + _esc(fail_updated)) if fail_updated else ''}
  </div>
  <div class="stats">
